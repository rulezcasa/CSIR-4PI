...
#Configure logging
logging.basicConfig(filename='logs/seaquest_train.log', level=logging.INFO, format='%(asctime)s - %(message)s')

#function to log metrics/system usage
def log_into_file(episode,episode_length,total_reward):
...
class ATDQNAgent:
    def __init__(self, action_size, state_shape, tau=0.5, beta_start=0.4, beta_end=1.0, T=1_000_000, device="cuda"):
...
for step in tqdm(range(total_steps), desc="Training Progress"):
    ...
    if done:
        ...
        if episode % 100 == 0:
            log_into_file(episode,episode_length,total_reward)
