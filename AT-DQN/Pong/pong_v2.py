...
#Configure logging
logging.basicConfig(filename='logs/pongv2_train.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
class ATDQNAgent:
    def __init__(self, action_size, state_shape, tau=0.45, beta_start=0.2, beta_end=0.8, T=20000000, device="cuda"):
    ...
    def normalize_attention(self):
        ...
        if min_val == max_val:
            ...
            self.alpha = defaultdict(lambda: 0.25, 
                                             {k: (v - min_val) / (max_val - min_val) for k, v in self.alpha.items()})
...
wandb.init(project="AT-DQN", name="Pong_v2", config={"total_steps": 20000000, "beta_start":0.2, "beta_end":0.8, "lr":0.00025, "tau":0.45})
...
model_path = "AT_DQN_Models/atdqn_pongv2_model.pth"
